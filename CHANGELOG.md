...
- **New agent-memory Markdown files were world-readable (0644), where they had been owner-only (0600)**
  ...
  - `semantica/context/_atomic_write.py`: `atomic_replace` / `atomic_write_text` take a `mode` for *new* files, defaulting to today's behaviour; `semantica/context/agent_memory.py` passes `0o600`. An existing destination still keeps its own mode either way, so an operator's chosen permissions still win
