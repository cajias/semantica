...
- **Snapshot persistence for `ContextGraph` — the graph the REST API and Explorer serve lives in memory, and until now a restart lost it**
  - New `semantica/context/snapshot.py`. `SnapshotStore` writes and reads one object holding one serialised graph, to a local filesystem path or to `s3://bucket/key`; `SnapshotService` restores at start-up, snapshots on an interval while the graph is dirty, and takes a final snapshot on shutdown, which is what makes an ordinary redeployment lossless
  - Serialisation is not reimplemented: both destinations route through `ContextGraph.save_to_file` / `load_from_file`, so the S3 object is byte-for-byte the local file
  - One asymmetry carries the design: *absence* of a snapshot means "first ever deployment" and yields an empty graph, while every other failure — denied read, missing bucket, truncated payload — raises. Starting empty after a permissions failure would let the next interval tick overwrite a good snapshot with an empty one, which is data loss disguised as a successful start-up
  - `SEMANTICA_SNAPSHOT_URI` selects the destination and enables the feature; unset means no snapshots, exactly as before. `SEMANTICA_SNAPSHOT_INTERVAL` (seconds, default `30`) is in effect the data-loss budget for an unclean exit. Both are read identically by `semantica-server` (`semantica/server.py`) and `semantica-explorer` (`semantica/explorer/app.py`), whose lifespans each run restore → start → stop
  - Restore replays the whole graph with mutation notifications suspended, so `semantica/explorer/session.py` gains `reload_graph()` — the search index and cached embeddings were built from an empty graph and would otherwise stay stale behind a restored one
  - `semantica/context/__init__.py` exports `SnapshotService`, `SnapshotStore`, `snapshot_store_from_env`, `snapshot_interval_from_env` and `suspended_mutations`
  - Both S3 directions stage the payload through a file under `TMPDIR`, so `TMPDIR` must be disk-backed and sized for the serialised graph — a hardened runtime capping `/tmp` at 64 MB fails every tick for a larger graph, safely but forever
  - New `pip install semantica[snapshot-s3]` extra (`boto3>=1.24.0`) — deliberately narrower than `cloud`, which also pulls azure/gcs that `snapshot.py` never touches — included in `all`, and installed by the `Dockerfile` alongside `explorer` so the stock image honours an `s3://` URI unmodified
  - New `tests/context/test_snapshot_store.py`, `tests/context/test_snapshot_writer.py`, `tests/context/test_snapshot_restore.py`, `tests/explorer/test_snapshot_lifespan.py`, and `tests/integration/test_aws_snapshot_roundtrip.py` (container-restart round trip, including a SIGKILL case that isolates the interval writer from the shutdown snapshot)
  - Docs: `docs/cli-setup.md`, `docs/reference/explorer.md` and `explorer/README.md` document both variables, their defaults, that an `s3://` URI needs `boto3`, that any other URI scheme is rejected at start-up rather than silently treated as a path, and that a restored snapshot replaces the graph loaded from `--graph`
- **AWS App Runner deployment for the Explorer, with the snapshot in S3** — `deploy/aws/`
  - `deploy/aws/README.md` is the runbook: bucket with versioning and a noncurrent-version lifecycle rule, the API key in Secrets Manager passed by reference, instance and access roles, image build, service create. `deploy/aws/apprunner-service.yaml` is the CloudFormation template (health check on `/api/health`, 1 vCPU / 2 GB, `SEMANTICA_API_KEY` injected as a runtime secret); `deploy/aws/instance-role-policy.json` is the container's own credential
  - Autoscaling is pinned to one instance on purpose: the graph is in-memory and every instance would write the same snapshot key, so a second instance silently overwrites the first
  - The container reaches storage through the instance role, so the deployment holds no long-lived access key
- **`docker-compose.yml` snapshots to a named `snapshot_data` volume by default**, so a `docker compose down && up` keeps the graph
  - The `Dockerfile` pre-creates and chowns `/data` rather than leaving it to the mount: Docker only propagates ownership onto a fresh named volume from a directory already present in the image, and a mountpoint it creates itself is `root:root 0755`, which the non-root `semantica` user cannot write

### Changed

- **`ContextGraph.save_to_file` now needs write permission on the destination's *directory*, not just on the file — a writable file inside a non-writable directory fails where it previously succeeded**
  - The write became atomic (temp file in the destination's directory + `os.replace`) so that a failure part-way through can no longer replace a good snapshot with a truncated one. `os.replace` needs a temp file on the same filesystem, hence the same directory, so the directory must be writable
  - This breaks two real setups that worked before: `docker run -v $PWD/graph.json:/data/graph.json` (a single-file bind mount into a root-owned `/data`) and Kubernetes `subPath` file mounts. Mount the *directory* instead of the file, or make the directory writable by the process uid
  - A symlinked destination is still written *through* rather than replaced, and an existing file keeps its own permission bits, so neither of those changes

### Fixed

- **On `semantica-server`, installing the snapshot writer left the Explorer's search index frozen at start-up state, so anything written after boot was invisible to `POST /api/graph/search`**
  - `GraphSession` reads "a `mutation_callback` is installed" as "someone else bumps the graph revision and maintains the search index", and `semantica/server.py`'s lifespan called `SnapshotService.start()` with the slot still empty, so the writer's dirty hook occupied it alone and nothing ever rebuilt the index. `semantica/explorer/app.py` was unaffected: its `_install_mutation_bridge` runs first, so the writer chains onto it
  - The session's own handler is now installed before `from_env`/`restore`/`start`, so the dirty hook chains rather than silences. Direct assignment rather than importing the Explorer's bridge: `semantica/explorer/app.py` builds a second FastAPI app at module scope, and this entry point mounts no WebSocket route for the bridge's broadcast half anyway
  - `tests/explorer/test_snapshot_lifespan.py::test_the_snapshot_hook_does_not_disable_index_maintenance`
- **The Explorer app — the one the Docker image and the App Runner service actually start — sent no security response headers at all**
  - `semantica/server.py` defined a private `_SecurityHeadersMiddleware` and `semantica/explorer/app.py` had none, so the internet-facing application shipped without `X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy` or `Permissions-Policy`
  - New `semantica/utils/security_headers.py` holds the single definition both apps now install, for the same reason `semantica/utils/cors.py` is the single CORS resolver. The static headers are byte-identical to what `server.py` sent
  - HSTS now keys off the first hop of `X-Forwarded-Proto`, falling back to `request.url.scheme`. TLS is terminated by the load balancer, so the request uvicorn sees is plain HTTP and the old scheme-only check meant HSTS was never sent on a deployed service. A forged header only produces an HSTS header on a plain-HTTP response, which browsers must ignore
  - `tests/explorer/test_cors_settings.py::TestSecurityHeadersReachBothEntryPoints` asserts both apps send every shared header and that HSTS follows the client-facing scheme, so installing the middleware on one entry point only fails the suite
- **The duplicate-merge route rebuilt the graph outside its lock and never announced the change, so a snapshot could capture a half-merged graph and the merge itself could go unsnapshotted**
  - `POST /api/enrich/merge` (`semantica/explorer/routes/enrich.py`) reassigned `graph.edges` and deleted from `graph.nodes` with no lock held, so the 30-second writer — serialising from its own thread — could observe the new edge list alongside the still-present duplicate node. It also emitted no mutation for the removal, so the writer's dirty flag stayed clear and the search index kept the merged-away node
  - The whole per-duplicate collapse now runs under `graph._lock` (an `RLock`, so the nested `graph.add_edges()` is safe), and a `REMOVE_NODE` mutation is emitted after the delete with the payload shape `purge_node` already uses. The existence guard moved inside the lock, removing a TOCTOU
  - New `tests/explorer/test_enrich_merge_snapshot.py`
- **A wrong-region or blackholed S3 endpoint stalled container boot for about five minutes and blew through the shutdown join budget**
  - `boto3.client("s3")` was built with botocore's defaults — 60 s connect, 60 s read, 5 attempts — which `restore()` spends stalling start-up and `stop()` spends waiting on the write lock, long past `STOP_JOIN_TIMEOUT`. The client is now configured with a 5 s connect timeout, a read timeout equal to `STOP_JOIN_TIMEOUT` and 2 attempts, dropping the worst case to roughly 45 s
  - `_load_object` also streams the response body straight to the staging file with `shutil.copyfileobj` instead of `read()`-ing it into memory first, halving peak usage on a path whose own docstring treats the staging copy as scarce
  - `tests/context/test_snapshot_store.py` gains `test_the_s3_client_is_built_with_a_timeout_budget` and `test_the_body_is_streamed_to_the_staging_file`
- **A `SnapshotService` restarted after `stop()` reported that it was snapshotting on an interval and then never ticked**
  - `stop()` sets `_stop` permanently and `start()` never cleared it, so the new thread's first `wait()` returned True and it exited immediately. `start()` now clears the event, and its re-entry guard checks `is_alive()` rather than "`_thread is not None`" — `stop()` clears `_thread` even when its bounded join times out, so the attribute check alone could leave the old thread running alongside a new one
  - `tests/context/test_snapshot_writer.py::TestThreadBehaviour::test_a_restarted_service_still_ticks`
- **The three snapshot start-up lines an operator needs — which destination is in use, and that the process is booting with an empty graph — were dropped from the logs entirely**
  - They were `logger.info`, and neither `semantica/explorer/app.py` nor the `semantica-explorer` console script configures logging, so under uvicorn's `dictConfig` the root logger falls back to `logging.lastResort` at WARNING and every INFO record disappears. All three are now WARNING and each still names the exact destination (via the store's `description`, so never a credential)
  - `tests/context/test_snapshot_restore.py::TestRestore::test_the_destination_is_logged_above_info`
- **An existing agent-memory Markdown file left at 0644 by an older build stayed world-readable through every subsequent write**
  - `atomic_replace` / `atomic_write_text` (`semantica/context/_atomic_write.py`) applied their `mode` only when the destination did not already exist, so `AgentMemory.export(format="markdown")` passing `0o600` fixed new files and silently left a pre-existing 0644 one — from an older build, a restore, or a tarball — readable by any other account on a shared host
  - `mode` is now `Optional[int]`: omitted keeps the previous contract (existing file keeps its bits, new file gets the umask default), while an explicit mode is enforced on every write. `semantica/context/agent_memory.py` passes `0o600`
  - `tests/context/test_agent_memory_markdown.py::test_overwriting_a_world_readable_markdown_memory_file_restores_0600`; `tests/context/test_context_graph_atomic_save.py`'s POSIX skip marker also moved onto the permission tests it belongs to, so the payload-detachment tests now run everywhere
- **An exported `SEMANTICA_SNAPSHOT_URI` made the test suite restore from, and snapshot to, the developer's own graph file**
  - Both lifespans call `SnapshotService.from_env`, so every `TestClient` in `tests/explorer/` picked the variable up: state bled between tests and the real file was overwritten. `tests/explorer/conftest.py` now unsets both snapshot variables for the session — session scope, not the function-scoped `monkeypatch`, because `tests/explorer/test_explorer_api.py` builds module-scoped clients whose lifespan runs first
- **`deploy/aws/` told operators to set a CORS variable the canonical resolver deprecates, and to hand-edit the `Dockerfile` before building**
  - `deploy/aws/apprunner-service.yaml` and `deploy/aws/README.md` set `ALLOWED_ORIGINS`, which the image already bakes into its own `ENV` — so the value passed by the template would have lost to it had the deprecated name won precedence. Both now use `SEMANTICA_CORS_ORIGINS`
  - The `boto3` pre-build step is gone: the stock image carries it via the `snapshot-s3` extra. The failure mode it described was also wrong — restore runs before uvicorn finishes starting, so a `boto3`-less image fails at start-up rather than degrading a healthy service
  - Step 1 gains a `put-bucket-lifecycle-configuration` call. Versioning without expiry grows forever: the writer rewrites the whole graph to the same key on every dirty tick, so a 30-second interval retains 120 noncurrent versions an hour and prunes none. `NoncurrentDays=7` is the only parameter set, bounding retention at seven days of write volume rather than at a version count, and needs no new IAM grant — S3 performs the expiry itself. `NewerNoncurrentVersions` is deliberately left unset: S3 expires a version only once it is both older than `NoncurrentDays` *and* has more newer noncurrent versions than that value, so it is a retention floor, never a cap — setting it can only keep more
  - Documented what setting `SEMANTICA_API_KEY` on this stack actually does: the bundled UI sends neither the header nor the WebSocket query parameter, so every data panel shows `HTTP 401` and the socket closes 4401 while `/api/health` reports healthy throughout. Drive it from REST clients or an authenticating reverse proxy — not `SEMANTICA_ALLOW_ANONYMOUS`, which publishes the graph. The CORS sentence was also wrong: the bundled UI is same-origin, so the origin list governs cross-origin callers and the `/ws/graph-updates` `Origin` check
- **A shutdown snapshot could race an upload still in flight and leave the *older* payload stored, silently, on the ordinary redeploy the feature exists to make lossless**
  - `SnapshotService.stop()` (`semantica/context/snapshot.py`) joined the interval writer with a 10 s bound and then took its final snapshot unconditionally, with nothing serialising the two. botocore's default read timeout is 60 s with retries, so one throttled `put_object` blows past that join and two PUTs race one key — and S3 resolves competing PUTs by *completion* order, so the stale payload can land last while `stop()` reports success. The container round trip cannot see it because its uploads finish in milliseconds
  - `snapshot_if_dirty()` now holds a `threading.Lock` across its whole body, so the final write queues behind any in-flight one and is newest-and-last by construction. The bounded join degrades to a warning rather than a decision. Deliberately not `graph._lock`: the payload is built under that lock and released before the upload, so a network round trip must not block every mutation
  - `tests/context/test_snapshot_writer.py::TestShutdown::test_the_final_snapshot_lands_after_an_upload_still_in_flight` reproduces the overlap deterministically with events (the first save blocks until a second one *starts*), so an unserialised implementation always completes them in the wrong order rather than only under an unlucky schedule
- **New agent-memory Markdown files were world-readable (0644), where they had been owner-only (0600)**
  - `AgentMemory.export(format="markdown")` moved onto the atomic writer shared with `save_to_file`, which gives a new file the umask-derived mode a plain `open()` would — right for a snapshot an operator reads, wrong for memory. Previously `NamedTemporaryFile` + `os.replace` left these files at tempfile's 0600. On a shared host or a multi-uid container, every new memory file became readable by any other account
  - `semantica/context/_atomic_write.py`: `atomic_replace` / `atomic_write_text` take a `mode`, and `semantica/context/agent_memory.py` passes `0o600`. Omitting it keeps today's behaviour, under which an existing destination keeps its own mode; passing it enforces the mode on every write, which is what a memory file needs
  - `tests/context/test_agent_memory_markdown.py::test_a_new_markdown_memory_file_is_owner_only` alongside the existing snapshot test asserting 0644 — both pass together, which is the point
- **Retraction and purge audit records were lost on every restart, so the `purged_at`/`reason` proof that an erasure happened disappeared on each redeploy**
  - `ContextGraph.save_to_file` never wrote `_retractions`/`_tombstones` and `load_from_file` cleared them. The retraction *effect* survived (it is expressed as `valid_until`, which `add_nodes` reads back, so nothing resurrects) but `list_retractions()`, `list_tombstones()`, `get_retraction()` and `get_tombstone()` all came back empty. Harmless while a round trip was an explicit user action; the interval snapshot writer makes it the automatic every-restart path
  - Both are now in the payload as **lists** of records, not objects: the in-memory key is the `(entity_kind, entity_id)` tuple JSON cannot express, and each record already carries both halves, so the key is rebuilt on load without inventing a separator an id could contain — the node and edge id keyspaces stay separate, as `context_graph.py` documents they must
  - Additive and backward-compatible: a snapshot lacking the two keys loads exactly as before, with no records
  - `tests/context/test_context_graph_retraction.py::TestSnapshotRoundTripPreservesRecords` — 5 tests covering retraction records, tombstones, the two id keyspaces under one shared id, restored idempotence, and the missing-keys compatibility path
- **A set `SEMANTICA_SNAPSHOT_URI` was silently ignored when there was no graph to snapshot**
  - `SnapshotService.from_env` returned `store=None` whenever `graph is None` and `restore()` returned before its log line, so the disabled case produced no output at all. Running `semantica-server` from an image without the explorer extra gives `EXPLORER_AVAILABLE = False` → `session = None` → snapshots off, server healthy, nothing logged, nothing stored — fail-*open* inside a module whose docstring makes fail-closed the governing principle
  - It now logs a WARNING naming the destination (via the store's `description`, so never a credential) and saying persistence is disabled and why. It does not raise: the server has legitimate routes that need no graph, and aborting start-up over it would be the worse trade
- **A mistyped snapshot scheme silently became a local path, disabling durability while reporting success**
  - Anything not starting `s3://` was treated as a filesystem path, so `s3:/bucket/key` (one slash), `S3://…`, `s3a://…`, `https://…` and `file:///…` all became local paths — `_save_file` would `makedirs` a directory literally named `s3:`, the service logged `context graph snapshots: file s3:/bucket/key`, snapshotted every 30 s and reported success, while writing the whole graph into the container's ephemeral layer and losing it on every redeploy
  - `SnapshotStore.__init__` now rejects a URI containing `://` that does not start with `s3://`, and the bare `s3:/` single-slash form, with a `ValidationError` naming both accepted shapes. A genuine filesystem path — relative, absolute or Windows-style — has no `://`, so those are untouched
  - `tests/context/test_snapshot_store.py::TestUriParsing` gains a parametrised rejection test over all five near-miss forms and an acceptance test over three plain paths plus a well-formed `s3://` URI
- **A second `SnapshotService` over one `ContextGraph` was silently non-persistent**
  - The idempotence guard `_snapshot_writer_installed` lives on the graph, `_dirty` lives on the service, and `stop()` never removed the callback — so when a graph outlived one service, the next service's dirty-marking callback early-returned, `_dirty` stayed False forever, and every snapshot including the shutdown one did nothing and reported success. Reachable here: `create_app()` builds `active_session` outside the lifespan, so the graph survives across lifespan runs of one app object, which two `with TestClient(app):` blocks produce
  - `stop()` now restores the `previous_callback` it captured and clears the flag, so a later service installs cleanly. Only the service that actually installed restores anything, and the captured callback is the Explorer's WebSocket bridge — installed before the writer, so it has to survive the writer being removed
  - `tests/context/test_snapshot_writer.py::TestShutdown` gains `test_a_second_service_over_one_graph_still_snapshots` and `test_stop_restores_the_previous_mutation_callback`
- **`ContextEdge.to_dict()` handed out the live metadata dict, so an in-place edit could tear a snapshot mid-serialisation**
  - `save_to_file` snapshots nodes, the edge list and node properties under the lock — `ContextNode.to_dict` copies — then releases it before `json.dump`, but the edge's `"properties"` was `self.metadata` by reference. A concurrent in-place mutation yields `RuntimeError: dictionary changed size during iteration` or a payload mixing pre- and post-mutation state. Latent (no live mutator on a served route today) but reachable now that the 30-second writer serialises alongside request handling
  - `ContextEdge.to_dict` now copies, matching the node sibling. Shallow, like the sibling: nested values stay shared, and a deep copy would be paid on every edge of every snapshot for a case no caller has
  - `tests/context/test_context_graph_atomic_save.py::TestSerializedPayloadIsDetached` pins both the edge fix and the node behaviour it restores parity with
